...
function build() {
  ...
  const mockDemoEnabled =
    process.argv.includes('--mock') ||
    process.env.WEBLM_MOCK_DEMO === '1' ||
    process.env.WEBLM_MOCK_DEMO === 'true' ||
    process.env.WEBLM_MOCK_DEMO === 'TRUE';
...
// Build flags (generated by scripts/build.js)
const WEBLM_MOCK_DEMO = ${mockDemoEnabled ? 'true' : 'false'};
...
`;
  ...
  // 注入 build flags 到 background（便于在 mock 构建时自动启用拦截）
  try {
    const bgSrc = fs.readFileSync(path.join(rootDir, 'background/background.js'), 'utf8');
    const bgWithFlags = `// Build flags (generated by scripts/build.js)\nconst WEBLM_MOCK_DEMO = ${mockDemoEnabled ? 'true' : 'false'};\n\n${bgSrc}`;
    fs.writeFileSync(path.join(distDir, 'background/background.js'), bgWithFlags);
    console.log('🧩 注入 background build flags');
  } catch (e) {
    console.warn('⚠️ background build flags 注入失败:', e);
  }
